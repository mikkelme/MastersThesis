...
if __name__ == "__main__":
    ...
    for i, filename in enumerate(filenames):
        ...
        c_Ff1, c_Ff2, c_Ff3 = savgol_filter(window_length, polyorder, c_Ff1, c_Ff2, c_Ff3)
        
        avgstep, c_Ff1, c_Ff2, c_Ff3 = avg_forward(interval, timestep, c_Ff1, c_Ff2, c_Ff3)
        fig, ax = plt.subplots(2, 2, num = i)
        fig.suptitle(filename)

        # Fx
        ax[0,0].plot(timestep, f_spring_force1, label = "spring force")
        ax[0,0].plot(avgstep, c_Ff1, label = "group/group force")
        ax[0,0].set(ylabel='$F_x$')
        ax[0,0].label_outer()

        # Fy
        ax[0,1].plot(timestep, f_spring_force2)
        ax[0,1].plot(avgstep, c_Ff2)
        ax[0,1].set(ylabel='$F_y$')

        # |Fxy|
        ax[1,0].plot(timestep, f_spring_force4)
        ax[1,0].plot(avgstep, np.sqrt(c_Ff1**2 + c_Ff2**2))

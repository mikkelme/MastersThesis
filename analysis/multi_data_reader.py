...
def read_multi_folder_old(folders, eval_rupture = False):
    for folder in folders:
        stretch_pct, F_N, Ff, rup, filenames, contact = read_multi_folder(folder, eval_rupture)
    ...
    #                 data.append((stretch_pct, F_N, metal_to_SI(fricData['Ff'], 'F')*1e9, rupture_score, job_dir, np.array([np.mean(fricData['contact'][0]), np.mean(fricData['contact'][1])]))) 
    ...
    #     rup = rup[stretch_args][:, FN_args]
    #     filenames = filenames[stretch_args][:, FN_args]
    #     contact = contact[stretch_args][:, FN_args]
...
def read_multi_folder(folder, eval_rupture = False):
    # Settings
    info_file = 'info_file.txt'
    friction_ext = 'Ff.txt'
    chist_ext = 'chist.txt'
    ...
    return stretch_pct, F_N, Ff, rup, filenames, contact
      

# def stability_heatmap(folders)
...
if __name__ == "__main__":
    ...
    read_multi_folder_old(folders, eval_rupture = False)
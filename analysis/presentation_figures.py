...
def contact_plot(filename):
    info, data = analyse_friction_file(filename)    
    time = data['time']
    COM = data['COM_sheet'][:,0]
...
def multi_plot_compare(folder1, folder2):
    ...
    # spec = gridspec.GridSpec(ncols=1, nrows=3, width_ratios=[1, 1, 1])
    ...
    # ax13 = plt.subplot2grid(grid, (0, 2), colspan=1)
    
    fig1, (ax11, ax12, ax13) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    fig2, (ax21, ax22, ax23) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    fig3, (ax31, ax32, ax33) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    fig4, (ax41, ax42, ax43) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    fig5, (ax51, ax52, ax53) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    fig6, (ax61, ax62, ax63) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    fig7, (ax71, ax72, ax73) = plt.subplots(1, 3, figsize = (10,5), gridspec_kw={'width_ratios': [1, 1, 0.1]})
    ...
    # ax23 = plt.subplot2grid(grid, (0, 2), colspan=1)
    ...
    # ax33 = plt.subplot2grid(grid, (0, 2), colspan=1)
    ...
    # ax43 = plt.subplot2grid(grid, (0, 2), colspan=1)
    ...
    # ax53 = plt.subplot2grid(grid, (0, 2), colspan=1)
    ...
    # ax63 = plt.subplot2grid(grid, (0, 2), colspan=1)
    ...
    # ax73 = plt.subplot2grid(grid, (0, 2), colspan=1)
    
    cmap = matplotlib.cm.viridis
    
    name = [folder.split('/')[-2] for folder in folders]
    figs = [fig1, fig2, fig3, fig4, fig5, fig6, fig7]
    ax = np.array([[ax11, ax21, ax31, ax41, ax51, ax61, ax71], 
                   [ax12, ax22,  ax32, ax42, ax52, ax62, ax72], 
                   [ax13, ax23,  ax33, ax43, ax53, ax63, ax73]]).T
    for f, folder in enumerate(folders):
        ...
        for j in range(len(F_N)):
            color = get_color_value(F_N[j], np.min(F_N), np.max(F_N), scale = 'log')
    ...
    for i, fig in enumerate(figs): # figures
        ...
        ax[i,2].grid(False)
        ax[i,2].set_aspect(10)
        fig.colorbar(matplotlib.cm.ScalarMappable(norm=norm, cmap=cmap), cax=ax[i,2], label=label)
        
        xlim = ax[i, 0].get_xlim() + ax[i, 1].get_xlim()
        xlim = [np.min(xlim), np.max(xlim)]
        ylim = ax[i, 0].get_ylim() + ax[i, 1].get_ylim()
        ylim = [np.min(ylim), np.max(ylim)]
        
        for j in range(2): 
            # l = ax[i,j].figure.subplotpars.left
            # r = ax[i,j].figure.subplotpars.right
            # t = ax[i,j].figure.subplotpars.top
            # b = ax[i,j].figure.subplotpars.bottom
            # figw = (r-l)
            # figh = (t-b)
            # ax[i,j].figure.set_size_inches(figw, figh)
            # print(f'({i},{j}) : {l},{r},{t},{b}')
        ...
        if i in [1, 2, 3]:
            fig.savefig(f"../Interview_presentation/figures/multi_{i}.pdf", bbox_inches="tight")

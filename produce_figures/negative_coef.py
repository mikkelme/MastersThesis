...
def new_coupling_plot(path, compare_path = None, save = False, add_path = None, add_stretch_range = None, markings = None):
    ...
    # Original coupling
    axes[1].scatter(strain_to_load(stretch[non_rup]), Ff[non_rup], marker = marker_coupled_FN, **size_and_edge, color = color_original, zorder = 10)
    
    # Add data coupling
    if add_path is not None:
        axes[1].scatter(strain_to_load(add_stretch[add_non_rup][add_stretch_map]), add_Ff[add_non_rup][add_stretch_map], marker = marker_coupled_FN, **size_and_edge, color = color_added, zorder = 10)
        
        
    if markings: # Add markings add certain strains
        markings_settings = {'marker': 's', 's': 100, 'alpha': 0.8, 'edgecolors': "black", 'color': 'tab:pink', 'zorder': 20}
    ...
    axes[1].scatter([], [], color = 'grey', edgecolors = 'black', marker = marker_const_FN, label = r'Non-coupled')
...
def strain_to_frame(strain, start = 15 + 2*5, strain_rate = 0.001, dump_freq = 10000, dt = 1e-3):
    # [start] = ps
    # [strain_rate] = 1/ps
    # [dump_freq] = step/frame
    # [dt] = step/ps
    frame_per_ps = 1/(dt*dump_freq)
    frame = (start + strain/strain_rate) * frame_per_ps
    return frame

...
def show_coordinates(shape = (6,6), save = False):
    # Build sheet 
...
def show_center_coordinates(shape = (6,6), save = False):
    # Build sheet 
    mat = np.ones((shape[0], shape[1])).astype('int')
    builder = config_builder(mat)
    Cdis = 1.461
    ...
    for j in range(0, shape[1], 2):
        ...
        for i in range(0, shape[0]+1):
            ...
            horline.append([x,y])
            circle = plt.Circle((x, y), center_radii,  color = color_cycle(9))
            ax.add_patch(circle)
...
if __name__ == "__main__":
    shape = (6, 6)
    show_coordinates(shape, True)
    show_center_coordinates(shape, True)
    plt.show()

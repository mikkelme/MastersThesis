...
mpirun -n 1 lmp -in produce_reset_files.in -var dt 0.001 -var config_data sheet_substrate -var relax_time 0 -var stretch_speed_pct 0.5 -var stretch_max_pct 0.05 -var pause_time1 5 -var F_N 6.24150907 -var pause_time2 5 -var drag_dir_x 0 -var drag_dir_y 1 -var drag_speed 0.05 -var drag_length 30 -var K 1.8724527210000002 -var root . -var out_ext _default
...
for file in *.restart; do    
    ...
    mkdir "$folder"    
    mv "$file" "$folder"/"$file"    
    echo "put some text in here" > "$folder"/job.sh    
done

...
def one_config_multi_data():
    ...
    variables = {
        ...
        "stretch_max_pct": 0.30,
        "drag_dir_x": 0,
        "drag_dir_y": 1
    }
  
    # Varying paramters  
    num_stretch_files = 31
    F_N = np.array([10, 100, 200, 300])*1e-9
    # F_N = np.linspace(10e-9, 300e-9, 6)
    
    print(f"Samples: {num_stretch_files} x {len(F_N)} = {num_stretch_files*len(F_N)}")
    print(f"Stretch: {np.around(np.linspace(0,variables['stretch_max_pct'], num_stretch_files), decimals = 2)}")

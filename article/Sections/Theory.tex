...
% plastic deformations (allthough this assumption of plastic deformations has been critizised \cite{CARBONE20082555}). The chemical friction arrises from adhesion between microscopic contacting surfaces, (with energy loss \hl{contribted} breaking and forming of bonds?).
...
Going from microscale to nanoscale, on the order of $10^{-9}$ meter, it has been
predicted that continnum mechanics will break down \cite{luan_breakdown_2005}
due to the discreteness of atomic-scale\footnote{\hl{Should I be more consistent with using eigher ``Nanoscale'' or ``Atomic scale'' or make a note clarifying that this is synonyms in practice?}} surfaces. In a numerical study by Mo et
al. \cite{mo_friction_2009} (considering asperity radii of 5-30 nm) it has been shown that the asperity area
$A_{\text{asp}}$ is in fact sublinear with $F_N$, which is explained by observation that not all atoms within the circumfereance of the asperity make contact with the substrate. By modelling the real area $A_{\text{real}} = N
A_{\text{atom}}$, where $N$ is the amount of atoms within the range of chemical
interaction range and $A_{\text{atom}}$ is the associated atom surface area,
they found a consistent linear relationship between friction and real contact
...
simulation gave a sublinear relatinship, even though the $F_{\text{fric}}
\propto A_{\text{real}}$ was maintained. 

This result emphasizes that the contact area is still expected to be play a major role on regarding nanoscale asperities, but it is simply the definition of contact area that undergoes a change when transistioning from micro- to nanoscale. However, considering the simulation setup of our numerical study, a flat sheet on a flat substrate, we cannot rely on asperity theories. With no asperities present it is unknown (I could not find anay articles on contact area for nanoflakes) whether the real contact area will be a dominant parameter. On the side, we might be able to create such a dependency by creating asperities through a strategic combination of kirigami inspired cuts and stretching of the sheet. Hence, we might hypothesize that such a transsition will contribute to significant change in the goverening mechanism of friction in the system.
\\
\\
Due to the general idea that the surfaces does not have structural asperities on an atomic scale, the friction can instead be modelled as a consequencse of the rough potential of the atomic landscape.
...
The standard FK model consists of a 1D chain of $N$ classical
...
% Often, the effects of inertia are neglected, and this is referred to as the static FK model. Inclusion of kinetic energy instead results in what is known as the dynamic FK model. \cite{FK2D}
...
Let us begin with the simplest commensurate case of $\theta = 1$ where the
...
% One way to reduce friction is through incommensurability, i.e. structural incompatibility between the sliding surfaces on the atomic level. This effect, often called structural superlubricity [3–5], has been observed experimentally in nanoscale contacts, for example in graphite [6].\cite{FK2D}
...
degrees of freedom and center of mass movement of the chain. Hence, at zero
...
What frictional properties to we expect to see at a nonescale. Numerical and experimental. Refer to theory for each element. Perhaps try to seperate MD and experimental results from each other 

It might not be easy to find experiments of graphene flakes on silicon substrate. 

\paragraph*{Qualitatively}
\begin{enumerate}
  \item Stick slip: We expect to see periodic stick-slip motion with period mathing the lattice constant(s) \cite{mo_friction_2009}. In AFM and SFA experiemnts, the stic-slip motion ten to transistion into smooth sloding when the speed exceeds $\sim 1 \mu$ m/s while in MD modelling the same transistion is observed in the $\sim 1 m/s$ region \cite{Manini_2016}. This 6 order of magnitude discrepancy has been largely discussed in [141-144 \cite{Manini_2016}] in connection with the simplifications in MD simulations. 
  %Not in agreement: Firstly, our calculations recover the stick-slip behavior of the lat- eral forces, characteristic of FFM sliding experiments. In particular, we find the correct qualitative dependence of stick-slip on the springs stiffness characterizing the cantilever-tip-flake coupling: soft springs allow for clean stick-slip behavior, while hard springs inhibit it. (Atomistic simulations of the sliding friction of graphene flakes)

  \item Static friction: Theoretically static friction is expected... 
  ...
  \item Oritentation. Due to commensurability it is expected that the friction will vary along different orientations. In a numerical study by Zhu et al. \cite{zhu_study_2018} they found that a graphene flake sliding on a gold substrate exhibit the highest friction when sliding along the armchair direction. (We chose to slide aliong the zigzag directio more or less by random choice)

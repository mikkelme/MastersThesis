...
As we zoom in yet another time, going from microscale to nanoscale on the order
...
asperity area $A_{\text{asp}}$ is in fact sublinear with $F_N$ but explains this
by the fact that not all atoms within the circumfereance of the asperity contact
phase is actually in contact. By modelling the real area $A_{\text{real}} = N
A_{\text{atom}}$, where $N$ is the amount of atoms within the range of chemical
interactions and $A_{\text{atom}}$ is the atom surface area, they found a
consistent linear relationship between friction and real contact area. Without
...
However, considering the simulation setup of our nmerical study, a flat sheet on a flat substrate, it is (adjective) to rely on asperity theories even though they might somewhat successfull at the nanoscale. 

Are there some nanoflake, island experiments to refer to here? Otherwise go straight to the FK models.
...
% First no adhesion (passivated by hydrogen) gives $A_{\text{asp}} = F_N^{2/3}$ as predicted by hertz theory but $F_f \propto F_N$ yielding $F_f \ne \tau A_{\text{asp}}$. 


% Include the part of \cite{mo_friction_2009} that talks about single asperity theories working for nanoscale.

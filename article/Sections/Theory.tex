...
\end{align*}
for wavenumber $k$ and particle mass $m$. Resonances will accour when the washboard frequency matches the phonon dispersion relation for $k = q$ or any of the harmonics $k = nq$, $n = 1, 2, 3, \cdots$ yielding
\begin{align*}
  n \Omega(v^+) &= \omega(k=nq) \\
  \frac{2\pi v^+}{a_b} &= \sqrt{\frac{4 K}{m}} \sin{(n\pi \theta a_b)} \\
  v^+ &= \sqrt{\frac{K}{m}} a_b\frac{\sin{(n\pi a_c)}}{\pi} \\
  v^+ &= \sqrt{\frac{K}{m}} a_b\frac{\sin{(n\pi a_c)}}{n\pi}
\end{align*}
(Make sense? Remember to deal with the absolute sign...) Thus when the chain slides with a velocity around resonance, the washboard frequency can excite acoustic phonons which will dissipate to other phonon modes as well.

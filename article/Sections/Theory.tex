...
friction, wear and lubrication between two surfaces in relative motion \cite[p. 1]{gnecco_meyer_2015}. In this thesis we will only concern ourselves with dry friction (no lubrication) without any wear. Tribological systems take place across a broad range of time and length scales, ranging from geological stratum layers involved in earthquakes \cite{kim_nano-scale_2009} to microscopic atomistic processes, as in the gliding motion of nanocluster of a
nanomotor \cite{Manini_2016}. 

This vast difference in scale gives rises to different frictional mechanism being dominating at different scale. On a macro scale the system is usally (tested) under large loads and speeds leading to high contact stresses and wear, while the micro/nano-scale systems on the other hand operate under relatviely low loads and speeds with negligible wear \cite{kim_nano-scale_2009} \cite[p. 5]{bhushan_2013}. While macroscale friction
is often reduced into a few variables such as normal load, material type, speed and surface roughness it is clear that micro/nanoscale friction cannot be
generalized under this simple representation. On a micro/nano scale the surface properties domniate the tribological propteries and the systems will additionally be sensitive to temperature, humidity and even sliding history. The works of Bhushan and Kulkarni (1996)\cite{BHUSHAN199649} showed that the friction coefficient decreased with scale, revealing the change in the mechanism of friction as the contact condition was altered, even for the same type of materisl used. 


The phenomenological descriptions of macroscale friction cannot yet be derived from the fundamental atomic principles and bridging the gap between different length scales in tribological systems remains an open challenge \cite{Manini_2016}. Hence, the following sections will be devided into macro, micro and nanoscale representing the different theories at each scale governing the understanding of friction. Our study is going to take place at the nanoscale, but since any application will eventually take action on a macroscopic level, the multi scale understanding serves as usefull way to bridge expectations into real life (does this make sense?)
...
Our working definition of macroscale is everything on the scale of visible everyday objects. We want to make a distinction to microscopic where the prefix indicates the size of micrometers $m^{-6}$ and hence essentially assign everything larger than that into the term macroscale\footnote{The width of a human hair is on the length scale $10^{-5}$ to $10^{-4}$ m which constitute a reasonable boundary between macro- and microscale as this is at the lower boundary of what we can percept.}.
...
% Based on \cite{gnecco_meyer_2015}
 ...
 Charles de Coulomb (1736-1806) all contributed to the empirical law, commonly known Amontons’ law, which states these fricitonal forces is entirely independent of contact area and sliding velocity (at ordinary sliding velocities). Instead, it relies only on the normal force $F_N$, acting perpendicular to the surface, and the material specific friction coefficient $\mu$ as
\begin{align*}
  F_{\text{fric}} = \mu F_N.
\end{align*}
The friction coeffcient is most often different for the cases of static $\mu_s$
and kinetic $\mu_k$ friction, usually with values lower than one and $\mu_s \ge
\mu_k$ in all cases [p. 6]\cite{gnecco_meyer_2015}. \\


Statistical mechanics provides a general explanation for the phenomena of friction \cite{Manini_2016}: A system in equilibrium has its kinetic energy uniformly distributed among all its degrees
of freedom (equipartition of energy). 

When a macroscale object is sliding in a given direction it is

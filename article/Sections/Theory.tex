...
\paragraph*{General formulation of potentials...?}
...
The Lennard Jones is probably one of the most famous pair potentials used in MD simulations. The Lennard-Jones (LJ) models the potential energy between two non-bonding atoms based solely on interatomc distance $r$. The model accounts for attractive forces arising from (dipole-dipole, dipole-induced dipole, and London interactions) and repulsive forces that capture the hard core (is this safe to say?) of overlapping wave functions at small distances. Thus is assummes neutrally charged atoms and was orginally proposed for noble gases. The classical 12-6 version of the model (refering to the powers of the repulsive and attractive form respectively) reads
\begin{align*}
  E = 4\epsilon \left[\left(\frac{\sigma}{r}\right)^{12} - \left(\frac{\sigma}{r}\right)^6 \right ], \qquad r < r_c,
\end{align*}

where $r$ is the interatomic distance with cut-off $r_c$, $\epsilon$ denotes the depth of the potential well and $\sigma$ the distance where the potential is zero. By solving for the potential minimum ($dE/dr = 0$) we find the equilibrium distance to be $r_0 = \sigma 2^{1/6}$, which makes it clear that sigma effectively sets the equilirbium distance between atoms, i.e. the dividing line for which the net force is repulsive or attractive. While the LJ in many ways is oversimplified model that is insufficient in its description of ... (get source and concrete examples) it is commonly used as a model for intermaterial interactions (between moving object and substrate) in friction studies [\cite{li_evolving_2016}, \cite{ZHANG201585}, \cite{kim_nano-scale_2009}].
...
The two-body term $\phi_2$ builds from the Lennard-Jones (LJ) model with the addition of an exponetial cutoff term
\begin{align}
...
The model parameters $A$, $\epsilon$, $B$, $\sigma$, $p$, $q$ and $a$ comes with $i,j$ indices to indicate that theese parameters should be specified for each unique pair of atom types. However, in our case we will only provide a single value for each model parameters as we are exclusively dealing with Si-Si bonds. We see that the first term in eq.~\eqref{eq:sw_2} is reminiscent of the Lennard-Jones potential while the last term effectively drives the portential to zero at $r=a\sigma$, which is the reason that we cut-off the model evaluation at that point. With the specific model parameters for the Si-Si modelling (see table \ref{tab:sw_param}) the cut-off becomes $\sim 3.8$ Å. \\
...
\begin{align}
  ...
  \label{eq:sw_3}
\end{align}
where $\theta_{0,ijk}$ is the equilibrium angle. The first term of eg.~\eqref{eq:sw_3} includes an angle dependency analog to a harmonic oscillator based on a cosine angle distance from the equilibrium angle. The final two terms act again as a cut-off by driving the potential to zero at $r_{ij} = a_{ij}\sigma_{ij}$ and $r_{ik} = a_{ik}\sigma_{ik}$. \\ 
The parameters used for the Si-Si bond modeling is displayed in table \ref{tab:sw_param} along with a (quick/superficial) interpretation of each model parameter.
...
\begin{table}[H]
  ...
  \begin{tabular}{ | c | c | L{9cm} |} \hline
    ...
    $\epsilon$ & 2.1683  & Effects the individual depth of the potential well for each atom type pair/tiplets. \\ \hline
    $\sigma$ & 2.0951 & Distance for which the two-body term is zero. \\ \hline
    $a$ & 1.80 & Effects the individual cut-off distance for each atom type pair. \\ \hline
    $\lambda$ & 21.0 & Effects the overall depth of the three-body potential well. \\ \hline
    $\gamma$ & 1.20 & Effects the shape of the three-body cut-off terms. \\ \hline
    $\cos{(\theta_0)}$ & -1/3 & Cosine of equilibrium angle. \\ \hline
    $A$ &  7.049556277 & Effects the overall depth of the two-body potential well. \\ \hline
    $B$ &  0.6022245584 & Scales the repulsion part of the two-body term. \\ \hline
    $p$  & 4.0 & Sets the power of the repulsion part of the two-body term. \\ \hline
    $q$  & 0.0 & Sets the power of the attraction part of the two-body term. \\ \hline
...
This section is baded on [\cite{docs_lammps_tersoff}, \cite{PhysRevB.37.6991}].

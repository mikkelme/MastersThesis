...
where $\delta_{\cdot\cdot}$ is the Kronecker Delta function and $\delta(\cdot)$ is the Dirac Delta function. It can be shown that a trajectory generated by integrating the Langevin equations of motion (eq.~\eqref{eq:Langevin}) maps a cononical distribution of microstates at temperature $T$. \cite{Hunenberger2005}(p.121). \\
...
\subsection{Fourier Transform (light)}

...
For the considerations of the columns in the population matrix, we define the count of 0's and 1's as $C_0(j)$ and $C_1(j)$ respectively. These are normalized as
\begin{align*}
...
% In addition, we define a survival probability $s_i = 1 - a_i$. Together, $a_i$ and $s_i$ decide the 


% In thus wau $a_i$ and $s_i$ decide together whether to mutate to the other state (flip binary) or to remain in the current state. We use $s_i$ as the statistical weight for the $i$-th chromosone given it a weight $w_i = s_i$.
% \\

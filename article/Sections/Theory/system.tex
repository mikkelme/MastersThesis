...
The tetrahedron pattern is defined in terms of center elements for which all atoms sourrounding a given center elements is removed. The pattern is characterized by two straight cuts, line 1 and line 2, perpendicular to each other which is translated to the whole sheet. We arrange the lines such that the center of line 1 alligns with the end of line 2. We define the pattern by the number of center elements in the first and second cut lines $L_1$ and $L_2$ along with a spacing between lines $sp$. In figure \ref{fig:pop_up} an example is shown for $(L_1, L_2, sp) = (7, 5, 2)$. 

Is the direction used for this pattern along the graphene lattice vectors? At least talk about this as one way to get perpendicular cuts. 



% and the number of center elements in spacing $sp$ between the lines. We arrange the lines such that the first line alligns with the center of the second line.


% cut units which is translated to the whole defined by a spacing. The cut pattern is defined 
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
     \caption{Deconstruction of the tetrahedron pattern with unit sizes $(7, 5)$ and spacing 2. $(a)$ The cut pattern highlighted in colors showing the translation of the first cut unit (green) to the whole sheet (light green) and similar the second unit in orange and light orange. The spacing between unit 1 and 2 is highlighted at two different spots in light blue $(b)$ The sheet after applying the cut pattern where the grey circles denotes atoms and the transparent white denotes empty positions. The small blue circles show the center elements for reference}.
     \label{fig:pop_up}
\end{figure}

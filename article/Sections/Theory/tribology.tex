...
\\
\\
From the review of previous results, we find several gaps and discrepancies
between the friction description provided by the reduced-models, \acrshort{MD}
simulations and experimental methods respectively. Some of these discrepancies
can be attributed to the fact that different physical mechanisms are included in
the numerical modelling that might not capture the relevant behavior found in
experiments. On the other hand, we might also point to the slight difference in
the studied stystems. For instance, the \acrshort{FFM} based results consider a
system where the tip is expected to deform under loading. From asperity theory
we can relate the loading to the increase in contact area which is considered a
key feature. Since the contact area is not observed to change under loading for
the atomic flat interface contacts this leaves a gap whether this can play an
important role still. By applying stretching to a Kirigami sheet we might
...
area. In addition, the deformation of the sheet will most likely be connected to
...
explanations being either true contact area or commensurability. The dependencies for velocity, temperature and spring constant provide a reference point for the possible friction regimes, such as stick-slip versus smooth sliding or ballistic versus diffusive motion, which is essential for a more through comparison and understanding of our results. 

...
% and \cite{gao_frictional_2004}
 ...
 surface we must overcome certain frictional forces $F_{\text{fric}}$ \cite{gnecco_meyer_2015}. The static friction force $F_s$ corresponds to the minimum tangential force required to initiate the sliding while the kintec friciton force $F_k$ corresponds to the
 ...
 Charles de Coulomb (1736-1806) all contributed to the empirical law, commonly known as Amontons’ law, which is a common base for our understanding of friction at macroscale level. Amontons’ law states that the fricitonal forces is entirely independent of contact area and sliding velocity. Instead, it relies only on the normal force\footnote{\textit{Normal force} is often used interchangeably with the terms \textit{load} and \textit{normal load} allthough the latter is an applied force while the latter is a reaction force of equal magnitude.} $F_N$, acting perpendicular to the surface, and the material specific friction coefficient $\mu$ as
\begin{align}
  F_{\text{fric}} = \mu F_N.
  \label{eq:amonton}
\end{align}
...
\vspace{0.1cm}
\\
\noindent The first definition (\ref{eq:mu_def1}) requires zero friction at zero load, i.e.\ $F_{\text{fric}} = 0, F_N = 0$, while the second definition (\ref{eq:mu_def2}) allows for a finite friction force at zero load and the coefficient is instead giving by the slope of the $F_{\text{fric}}$ versus $F_N$ curve. However, in reality the friction coeffcient is often found to vary under different conditions such as humidity or smooth and rough morphologies of the sliding surfaces \cite{gao_frictional_2004}.


%  \ref{eq:mu_def}


% In addition, for adhesive suraces the friction force is usually not zero under zero load which makes the definition in eq.~\eqref{eq:mu_def2} the common choice. Especially, if the amontos break down and the linearity is not present we might talk about a changing COF and thus eq.~\eqref{eq:mu_def2} is the only reasonable choice.  
...
Amontons' law has been surprisingly successful in is description of the majority of rubbing surfaces, involving both dry and lubricated, ductile and brittle and rough and smooth (as long as they are not adhesive) \cite{gao_frictional_2004}. However, it is known to that eq.\eqref{eq:amonton} is not valid over large ranges of loads and sliding velocity. The kinetic friction independency of sliding velocity dissapears at low velocities as thermal effects becomes important and for high velocities due to intertial effetcs. \cite[pp. 5-6]{gnecco_meyer_2015}. 
...
% All the terms in Amontons’ law refer to macroscopic, i.e., space- and time-averaged or “mean-field”, values. Thus, the contact area is the “apparent” or projected geometric area rather than the “real” contact area at the molecular level. And V is the mean relative velocity of the sliding bodies even though the shearing microjunctions may be moving with large fluctuations or in a stick-slip fashion.10 Early attempts to account for Amontons’ law, first by Amontons4 and later by Coulomb (1736-1806)11 and Euler (1707-1783),12 were based on analyses of how the microscopic surface asperities of one surface would have to climb over those of the other to allow for two surfaces to slide past each other... \cite{gao_frictional_2004}

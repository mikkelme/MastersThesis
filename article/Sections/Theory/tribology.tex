...
\begin{table}[H]
  ...
  \parbox[t]{2mm}{\multirow{11}{*}{\rotatebox[origin=c]{90}{AFM}}} & \multirow{1}{*}{Exp.} & 2015 \cite{Paolicelli_2015} & G. Paolicelli et al.\ & Graphene on SiO2 and Ni(111) substrate  & Layers, load, shear strength \\ \cline{2-6} 
...
method to simulate a graphene flake on an infinite graphene sheet
\cite{penkov_tribology_2014}. They implemented a FKT-like setup where each atom
in the flake is coupled horizontally to a rigid support by elastic springs,
however excluding thermal excitations. They recovered the stick-slip behaviour
which is also observed in \acrshort{FFM} setups both experimentally
\cite{zhang_tuning_2019} and numerically \cite{li_evolving_2016}, and they found an agreement with the qualitative observation that soft spring allows for clean stick-slip while hard springs inhibit it (below 40 nN/m?). In \acrshort{AFM} and SFA \hl{Not mentioned before} experiemnts, the stick-slip motion tend to transistion into smooth sliding when the speed exceeds $\sim \SI{1}{\mu/s}$ while in MD modelling the same transistion is observed in the $\sim \SI{1}{m/s}$ region \cite{Manini_2016}. This 6 order of magnitude discrepancy has been largely discussed in connection to simplifying assumptions in MD simulations Moreover, Bonelli et al.\ \cite{bonelli_atomistic_2009} found that commensurability, through orientation of the support and the angle of pulling, to be of high importance for the frictional behaviour which generally alligns with the predictions of the \acrshort{FK} type
models. They confirmed qualitatively the experimental observation of
superlubricity for certain incommensurable orientations as shown experimentally by Dienwiebel et al.\cite{DIENWIEBEL2005197} and further supported by experimental measurements of interaction energies by Feng et al.\ \cite{feng_superlubric_2013}. Although, the friction froce and friction coefficeint is found to be one order of magnitude higher than that of the experimental results which is attributed to the details of the numerical modelling. Experimental studies. Generally the experimental found coefficients between graphite and most materials lies in the range 0.08-0.18 \cite{DIENWIEBEL2005197} and while Dienwiebel et al. \cite{DIENWIEBEL2005197} reported very different frictional forces on the order 0.01-0.45 nN (all the way down to inmeasureable by their equipment) in a load range of roughly 30 nN, the change in friction with applied load was as low as 0.05-0.4 \% (coef of 0.0005-0.004). 

The experimental studies generally give coefficients in the range $\sim [0.01, 0.2]$ \cite{penkov_tribology_2014} \cite{DIENWIEBEL2005197} (what definition?)

% Talk about which direction is most likely to give superlubricity.
...
decreasing the rotational freedom as size increases. However, a numerical \acrshort{MD} study of monolayer islands of krypton on copper by Reguzzoni and Righi \cite{PhysRevB.85.201412} reports that the effective commensurability drastically below a critical flake radius on the order of $10$ Å. This supports the idea that increasing flake size will decrease friction though a reduction in commensurability, which is also in agreement with numerical studies by Zhu and Li \cite{zhu_study_2018}. In a numerical study
...
the edge is responsible for the pinning effect. Additionally, the edge pinning effect was found to decrease with increasing temperature as the edge energy barriers are reduced. In general this alligns with the \acrshort{FK} models prediction a lowering of friction due to thermal activation of the atoms overcoming the energy barriers which is also found the numerical study by Wijn et al.\ \cite{Wijn_2011} 


Evolution effects, or fiction strengthening, were observed experimentally by Zhang et al.\ \cite{zhang_tuning_2019} (only when on substrate) and numcerically by Li et al.\ \cite{li_evolving_2016}, meaning that the friction force will increase during the initial period of sliding. This is atrributed to ... and was found to dimish for an increasing number of layers in the substrate and was observed only when the two-dimensional material was loosely adhering to a substrate. On the same note they found in general that the friction between graphene and graphite increased with increasing layers which is also supported by the findings in \hl{XXX}

In an experimental study of suspended monolayer graphene by Zhang et al. \cite{zhang_tuning_2019} they found that straining the graphene sheet increased the dynamic friction 

% straning the sheet effectively modulating the flexibility which consequently change the local pinning capability of the contact interface resulting in a strain dependent frictional behvaiour. "suggesting a strategy for dynamic friction tuning via regulating atomic scale interface contact quality". The dynamic friction was reduced by increasing strain. 


% "The evolution effect, or friction strengthening, was proposed to originate from the evolving contact quality as the tip slid on 2D materials loosely adhered to a substrate (19)" \cite{zhang_tuning_2019}
...
\begin{table}[H]
  ...
  \label{tab:}
  \begin{tabular}{  M{3cm}  X{10cm} } \hline
  \textbf{Stick slip} & Generally we expect to observe periodic stick-slip motion with a period mathing the lattice constant(s) involved \cite{mo_friction_2009}. This is however inhibited for high stifness of the springs to the coupled support and at large sliding velocity. Flake size and temperature? \\ \\
  \textbf{Static friction} & The static friciton is highly related to the presence of stick-slip motion. The static friction is most pronounced for commensurable configurations. Reduce with friction? \\ \\
  \textbf{Commensurability} & Both static and dynamic friction is expected to be highly sensitive to commensurability, both through lattice spacing and orientation but also by the path the flake is forced along. A low spring stiffness will effect the freedom to shift around which affects commensurability. \\ \\
   \textbf{Friction evolution} & For a monolayer graphene sheet on a substrate \\ \\
  \textbf{Thickness} & Friction increase with increasing graphite layers (\hl{Is this relevant?}) \\ \hline
  \textbf{Negative coef} & Observed, or only with subtracting force \\ \hline
  \end{tabular}
  \end{center}
\end{table}

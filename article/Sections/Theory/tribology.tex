...
% For load speed thing on different scale on "typical experiments to avoid trouble with saying it for the general scale
...
Generally, there exist two temperature regimes in the model: \textit{Thermal activation} at low temperatures and \textit{thermal drift} at high temperatures as shown in \cref{fig:PT_temp}. At lower temperatures, the system is subject to standard thermal activation with $\Delta V_{j \to i} \gg \Delta V_{i \to j}$ resulting in  $\kappa_{j \to i} \ll \kappa_{i \to j}$. Effectively, this inhibits any backward slip and we get  
\begin{align*}
...
et al., they reported a general decrease in friction with an increasing number of layers which is found in other \acrshort{MD} studies \cite{Yoon2015MolecularDS} and experimental studies as well \cite{Filleter_2009, Lee_2010}. However, we also found a \acrshort{MD} study reporting the opposite trend \cite{Reguzzoni_2012}.
...
physical conditions related to the environment and the probing of friction. We aim to study the frictional behavior of a Kirigami sheet as it exhibits out-of-plane buckling under tension. The theoretical understanding indicates that such deformations will constitute a transition between two types of systems, an
asperity system and an atomically flat interface, which have only been
studied separately. Thus, our primary objective is to explore the effects of such a transition. In particular, our study aims to investigate the significance of contact area and to test the hypothesis that reducing the contact area results in decreased friction. Additionally, we seek to enhance our comprehension of how Kirigami sheet friction is affected by stretching and how this impacts its load dependence. To contribute with new insight into the field of friction we are especially interested in non-linear dependencies between friction and stretching of various Kirigami designs. From these results, we want to investigate the prospects of achieving a negative friction coefficient for a system of coupled load and stretch. Do the extensive list of important physical
parameters, we do not carry out an exhaustive evaluation of how our system behaves across the corresponding frictional regimes. Instead, we aim to create a \acrshort{MD} simulation which provides a good basis for the measurement of friction. The relevant understanding of how the physical conditions affect the simulation will be considered in order to frame our system within the theoretical understanding.


In order to get a broader view of the possibilities within Kirigami design for friction control we create a \acrshort{MD}-based dataset regarding the friction behavior for a variety of Kirigami designs. By the use of Machine learning, we want to investigate whether it is possible to pick up any useful trends in the dataset regarding the friction behavior as a function of kirigami design, stretch and load. This will serve as an initial clue whether this machine learning can be useful for this kind of problem as well as a tool for performing an accelerated search of Kirigami designs. Especially we want to use the data for an evaluation of the prospects of minimizing and maximizing the friction force and also the friction coefficient for a coupled system of load and stretch. 
...
% Initially, we want to focus on a qualitative description of the effect on out-of-plane buckling and consider the relation to contact area. We will then evaluate the dependence on stretch and investigate how this effect the load dependence and the associated frictional coefficients. 


% From there on, we want to create an dataset of multiple Kirigami patterns in order to explore the relation to stretching of the sheet. The aim is to identify the possibility for interesting non-linear effects. By training a machine learning model we will extend this search further in an accelerated manner. This exploration can be considered as a broad view of the possible effects from Kirigami induced deformations which will set the scene for further studies of the phyical mechanisms behind. Thus we consider our work as a proof of concept for the general method and as a relevant insight in a system domain not systematically studied. 
...
\begin{enumerate}
  ...
  \item What friction-related effects can be observed for a Kirigami sheet as it is stretched?
  \begin{enumerate}
    \item How does friction dependent on changing contact area?
    \item Are stretching effects significant for a non-cut sheet?
    \item How does the load dependence relate to stretched Kirigami sheets?
  \end{enumerate}




  \item  How can we design an \acrshort{MD} simulation that provides a stable foundation for an investigation of friction behavior for a Kirigami sheet sliding on a substrate? How do the Kirigami pattern and physical conditions such as strain, load, temperature and sliding speed control the frictional behavior?
  \item Can we generate a variety of Kirigami patterns to control the out-of-plane buckling?
  \item 
  
  but also some randomized designs for an improved variety in the machine learning data?
  \item Are we able to capture general trends in the friction data related to Kirigami designs, stretch and load using machine learning?
  \item Can we use a machine learning model to predict new designs optimizing for a minimum or maximum friction force and a minimum or maximum friction coefficient in a system with coupled load and stretch?
  \item What are the prospects of achieving a negative friction coefficient for a coupled system?
\end{enumerate}
...
%   \item Define a sheet indexing that allows for an unique mapping of patterns between a hexagonal graphene lattice representation to a matrix representation suited for numerical analysis. 
...
%   \item Perform an accelerated search optimizing for interesting frictional properties using the \acrshort{ML} model. This should be done both through the attern generation procedures and by following a genetic algorithm approach. 
...
%   \item Study certain designs of interest with the scope of revealing underling mechanisms. This includes simple correlation analysis but also a visualization of feature and gradient maps of the \acrshort{ML} network.
% \end{enumerate}

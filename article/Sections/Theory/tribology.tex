...
dominating at different scales. On a macro scale the system is usually subject
...
The phenomenological descriptions of macroscale friction cannot yet be derived from the fundamental atomic principles, and bridging the gap between different length scales in tribological systems remains an open challenge \cite{Manini_2016}. Hence, the following sections will be organized into macro-, micro- and nanoscale representing the theoretical understanding governing each scale regime. Realising that the field of tribology on all scales is a vastly  broad and intricate topic, we will aim to introduce the essential findings for each scale, while keeping a focus on the nessecary background to understand the dynamics of our system which takes place at the lowest scale.
...
macroscopic area \cite{kim_nano-scale_2009}. The adhesion model proposed by Bowden and Tabor \cite{bowden2001friction} is based on the fundamental reasoning that friction is governed by the adhesion between two surfaces. Since adhesion is proportional to the real contact area, the model predics that friciton is proportional to the area and the interfacial shear strength $\vec{\tau}$ between two contacting junctions \cite{Kim_2012}
\begin{align*}
...
We start with the Prandtl–Tomlinson (\acrshort{PT}) \cite[2,3]{Yalin_2011} with added thermal activation as proposed by Gnecco et al. \cite{PhysRevLett.84.1172}. For the theoretical foundation of this section  we generally refer to  \cite{Yalin_2011}.

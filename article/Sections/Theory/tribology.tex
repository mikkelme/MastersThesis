...
% η can be controlled by the variation of the normal load on the contact, which changes the potential corrugation U0 more than the contact stiffness.
% https://arxiv.org/pdf/1112.3234.pdf
...
multiple atoms in contact with the sample, all methods are relatable to the study of nanoscale surface contact. However, the \acrshort{FFM} type is more reminiscent of asperity theory as we expect it to deform with increasing load, while the latter two are more aligned with the \acrshort{PT} type models and our system. Nonetheless, we will consider results across all three types of systems with the relevant studies summarized in \cref{tab:friction_ref} for convenience. 
...
% Sircar and Patra \cite{Sircar_2020} rolling friction, generalized PT model says smooth sliding for high speed
...
simulations. On the other hand, the \acrshort{PT} model qualitatively disagrees with this as it predicts the reverse: high speed leading to the multiple slip regime. \hl{Little bit unsure what to say about this one.}



% This is certainly associated with stick-slip
...
% The waveguide has a speed cutoff: smooth sliding ceases below ,15 m/s, a typical velocity for transition to stick-slip motion in atomic-scale simulations30. \cite{liu_high-speed_2014}
...
% Temperature
Regarding temperature, the general experimental trend is decreasing friction
...
\\
\\
From the review of previous results, we find a gap between the friction
description provided by the reduced-models, \acrshort{MD} simulations and
experimental methods respectively. While some of the discrepancies can be
attributed to a difference in the modelling of physical mechanism, some might
also be attributed to differences in the considered system. For instance, the
\acrshort{FFM} based results considers a system where the tip is possible
deformed during loading. From asperity theory we can relate the loading to an
increase in contact area which is considered a key feature. Since the contact
area is not observed to change under loading for the atomic flat interface
contacts this leaves the question: What will happen if we are able to reduce the
contact area through Kirigami cuts and stretch of the sheet? Such a transition
represent a system change from an atomically flat system towards an asperity
system. Following asperity theory we might expect the friction to reduce with
decreasing contact area. On the other hand, commensurability is generally expected to be a dominant parameter for the friction behavior, and thus we would expect the deformation of the sheet give rise to a commensurability effect as well. This could both lead to a increase or an decrease in friction depending on whether we transition from an incommensurable to commensurable state or vice versa. For a Kirigami modified system the dependence on load will bring insight to the discussion of how friction depends on load with two governing explanations being either true contact area or commensurability. Furthermore, we must consider our results in terms of different friction regimes such as stick-slip and smooth sliding, ballistic and diffusive motion and thermal something. 
...
\begin{itemize}
  \item How does the contact area relate to friction as the sheet buckles during stretching. 

...
appropiate learning rate choice, and thus we perform a new learning rate range
test to determine a suitable choice for the learning rate in this grid search.
We propose two learning rate schemes: A constant learning rate as used until
this point and one cycle policy. The one cycle policy starts from a $1/20$ part
of the set learning rate and increase towards that during the first 30\% of
training. It then decreases towards a $1e-4$ factor of the set learing rate
during the remaining 70\% of training. The increase and increase is done by a
cosine function. The suggested learning rate for the constant learning rate
scheme/policy is determine by the steepest slope on the loss curve as earlier,
while for the one cycle policy the choice serve as max bound of a stable
learning rate. We find that the minimum point in the loss curve is as suitable
choice that approaches the diverging point without getting to close and causing
diverging learing. The learning rate range test for momentum is shown in \cref{fig:LR_range_mom}.

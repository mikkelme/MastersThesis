...
We take a look at how the ranking of the four properties of interest is ranked with the \acrshort{ML} model as seen in \cref{tab:ML_ranking_pop} \cref{tab:ML_ranking_hon} \cref{tab:ML_ranking_RW}. Generally the ranking for the minimum friciton is deviating the most. We also see negative values in the \acrshort{ML} prediction which hints that the sensitivity to the low values is not good enough. Otherwise, the \acrshort{ML} is pretty much able to get the top 5 rights for the remaning categories for the tetrehedron and honeycomb pattern. For the random walk it struggles a bit more. Looking at the values for the top candidates in the max-types properties we see that it is generally within a $\sim 0.2$ nN range. 
...
Judging from the ranking in \cref{tab:ML_ranking_pop} \cref{tab:ML_ranking_hon} \cref{tab:ML_ranking_RW}, the \acrshort{ML} model showcases a reasonable performance with respect to giving a qualitative pointer towards interesting configurations. 

...
detection. In combination this let us produce friction vs.\ stretch curves with
the rupture detection used to predict the stopping point of the curve in terms
of a maximum stretch value. However, it has often been proven useful to
introduce more variables in the output in order to strengthen the network
(\hl{get source}). In addition, this gives us more option for exploring the
relationship in the data later on. Thus, we include maximum friction, contact
count, porosity and rupture stretch in the data. Notice that rupture stretch
...
the performance of the rupture prediction to be satisficatory without
introducing such formalities. Notice that we weight the importance of these
variables differently as explained in the section about the loss function. 

\subsubsection{Data augmentation}
Due to the symmetry in the configuration, we can flip the configuration by
relfecting the configuration across the y-axis. We cannot do this across the
x-axis as the sheet is always slided in a positive y-direction and thus we
cannot guarantee that the results would be the same whether we slide backwards
or forward. We definitely expect a snow plow to perform differently when going
backwards and thus by analogy we would expect the direction of sliding with
respect to the configuration to be of importance. 



\subsection{Loss}
We use a rather standard loss function for the evaluation of our model
performance during training and validation. For the scalar values we use the
Mean Squared Error (\acrshort{MSE})
\begin{align*}
  L_{\text{MSE}} = \frac{1}{N} \sum_{i = 1}^N (y_i - \hat{y}_i)^2,
\end{align*}
where $N$ is the number of data entries and $y$ are the true variables and
$\hat{y}$ are the predicted values. For the prediction of rupture with
True/False labels we use binary cross entropy 
\begin{align*}
...
We calculate the total loss as a weighted sum between the loss associated with
each variable
\begin{align*}
  L_{tot} = \sum_{v} W_v\cdot L_v.
\end{align*}
We choose the weights to be $1/2$ for the mean friction and $1/10$ for the
remaining 5 variables thus sharing weight evenly for the remaining 50\% of the
loss. During the introductionary phase of the training we tried varying these,
but we found little change in the results and concluded immediately that the
precise choice of this weighting is not immensely important. 

\subsection{Hypertuning}
We build a series of models by VGGNet type. The first part consist of
convolutional layers and then final part of fully connected layers. Each
convolutional layer is followed by a Batchnorm, then a Relu and finally a
maxpool. As we go deeper into the network we increase the number of channels in
the convolutional layers, and similary we decrease the number of nodes in the
fully connected layers towards the end. We choose an even distribution of C and
FC layers. We do so in a staircase kind of way such, as for instance 
\begin{align*}
  \text{Input} \to \underbrace{\overbrace{C4}^{S \ = \ 4}C8 \ C16 \ C32 \ D32 \ D16 \ D8 \ D4}_{D \ = \ 8} \to \text{Output}
\end{align*} 

By varying the number of start channels $S$ and the depth $D$ we perform a
architecture search. However, the appropiate learning rate will depend on the
specific architecture. Hence, we perform a learning rate range test for each of
the suggested model architectures before doing the search. This is shown in
figure \cref{fig:LR_range}.
...
We then use these suggested learning rates for the architecture search. We look
at the total validation loss and the validation R2 score for the mean friction.
This is shown in \cref{fig:A_search_perf} 
...
Now look at the result for the more difficult patterns as a pseudo validaiotn
(these is actually expected to be partly in the training set already).

We look at the honeycomb and tetrahedron patterns with top 10 scores for maximum
drop in friction. This is chosen as a attempt to capture the ones with the most
complex non-linear trends in the stretch curve which seems to be both difficult
to predict but also of most interest for the study. This is shown in
\cref{fig:A_search_compare}.
...
From \cref{fig:A_search_perf} we found that models like S(8-32)D(8-12) gave
reasonble results. When looking at the selected validaiton set in figure
\cref{fig:A_search_compare} we godt considerable lower scores, especially for
the Tetrahedron pattern. Here the best peforming models were in general more
complex with S32D12 be the best candidate for the tetrahedron and S32D12 the
best for the honeycomb pattern. This is compatible with the general performance
from \cref{fig:A_search_perf} and by favoring the performance on the tetrahedron
set (which seemed to be more difficult) we settle on the S32D12 model.
...
% The results from LR momentum: momentum  [0.99, 0.97, 0.95, 0.9] lr_max
% [0.19098169151133398, 0.07099694630794687, 0.048523309416493646,
% 0.0078083987538332695]

...
As an attempt to get further insight into the model predictions, we use the Grad-CAM method to examine the top-scoring individuals from the genetic algorithm search. The result for the mixed porosity search is shown in ~\cref{fig:GC_mixed_p}. For a
...
\crefrange{fig:GC_pop_search}{fig:GC_hon_search}. For the mixed porosity configuration, the Grad-CAM method highlights some areas in the noise 

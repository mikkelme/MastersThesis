...
rather consistent result regarding the minimization of friction where the top
...
\hl{How many point for the stretch curce, what interval and what normal force?}
...
During the search, it became apperent that the predictions are highly sensitive
...
\hl{Looking at the actual curce the big difference in the numbers seem to be due to where the model predicts a rupture. The curve goes up and down again for most of them, but if the rupture is predicted before it drops it not giv any drop value. }
% Talk about high relative error for low friction which might be connected to poor performance in the low friction area.
...
Genetic algorithm search from Tetrahedron $(1,7,1)$ or Honeycomb $(3,3,5,3)$ did seemingly not produce any upgrades. The generated version (carying ref. pos.) with highest score remained like that for at least 180 generations  

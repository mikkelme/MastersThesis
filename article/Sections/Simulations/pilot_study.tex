...
From the investigation of the stretch dependency we saw that increasing the normal load from 0.1 to \SI{10}{nN} did not make a considerable impact on the friction in comparison to the effect associated with stretch. One special feature of our system is that we only apply load to the pull blocks. Thus, we investigate the friction under variation of normal load for a non-cut sheet with loading both on the pull block and with a more traditional uniform loading as shown in \cref{fig:load_dist}. The results do not provide any clear difference between the two load variations as both indicate a seemingly non-dependent relationship to friction considering the relative size of the errors. 
...
We investigate the dependency to normal load for the kirigami patterns as well at different stages of their stretching relative to their respective rupture point as shown in \cref{fig:load_dependency}. On a logaritmic load scale we see some increase with load for the non-cut sheet which is definitely sublinear. Notice that we increased the normal load range to 0.1--100 nN. As we approach 100 nN the non-cut sheet does increase faster with normal load closer to a linear trend. For the kirigami patterns the friction is more or less constant wit load for all stages of stretching. This leads to the conclusion that the load has a minor effect on our system in these conditions. 
...
\hl{Give some quick estimates for the size of the friction coefficeint perhaps}

...
As we discuss the numerical and physical dependencies in the system we also consider the computational cost. This especially relates to parameters as timestep, sliding speed and sliding distance which effect the number of computations done in a single simulation. 
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \end{subfigure}
    \hfill
    \caption{Computational cost related to temperature, sliding speed, spring constant and dt parameter. Sliding speed follows $t \propto v^{-0.977 \pm 0.005}$ and dt follows $t \propto \text{dt}^{-0.87\pm 0.02}$}
    \label{fig:comp_cost}
\end{figure}

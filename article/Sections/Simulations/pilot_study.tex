...
\begin{table}[h]
  \begin{center}
  \caption{Parameters involved in the numerical \acrshort{MD} simulation for measuring friction. The default values correspond to the final choice used for the \acrshort{ML} dataset for which the shaded cells denote the parameters varied.}
  ...
  \begin{tabular}{ | M{4cm} | M{3.5cm} | X{8cm}|} \hline
    ...
    \cellcolor{black!7} $F_N$ & \cellcolor{black!7} [0.1, 10] nN & \cellcolor{black!7} Applied normal load to the pull blocks. \\ \hline
...
no strain applied and a normal load of $\SI{1}{nN}$. 
...
the sliding direction shown in~\cref{fig:drag_Ff}. We use a sample rate of $\SI{10}{ps^{-1}}$ corresponding to one sample for each 100 timesteps, with each sample being the mean value of the preceding 100 timesteps. We observe immediately that the
...
data points (corresponding to a sliding distance of \SI{3}{Å} or a time window of
...
\begin{figure}[!htb]
  ...
  \caption{Running relative error calculated from the standard deviation (std) for a full \SI{400}{{\text{Å}}} sliding simulation. The running std window is \SI{70}{\text{Å}} (35\% of the running mean window, which is 50\% of the full sliding length).}
...
%     \centering
%     \includegraphics[width=\linewidth]{figures/baseline/contact_vs_stretch.pdf}
%     \captionof{figure}{The relative contact for a strained sheet in contact with a substrate. The relative contact is given as the relative number of atoms in the sheet within chemical interaction range. The cut-off for the interaction range is \SI{4}{\text{Å}} corresponding to $\sim 120 \%$ the \acrshort{LJ} equilibrium distance. No normal force is applied and the temperature is kept at $T = \SI{300}{K}$.}
%     \label{fig:contact_vs_stretch}
%   \end{minipage}
%   \end{figure}
...
From the temperature investigation in~\cref{fig:var_temp} we find an overall increasing
...
proportional as the friction force suddenly dips down and up again, in the
...
To examine the relationship between friction and normal load as the Kirigami
...
From the friction measurements in~\cref{fig:multi_load_fric} we find that the
...
\begin{align}
  ...
  \label{eq:mu_strain}
\end{align}
When considering the ratios $\Delta F_{\text{fric}} / \Delta \epsilon$ for the reduction in friction with strain for the Tetrahedron and Honeycomb patterns in~\cref{sec:stretch_dependency} we find the corresponding coupled system friction coefficients to be 
\begin{align}

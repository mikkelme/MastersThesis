...
We subdivide the two main parts of the simmulation, the sheet and the substrate, into specific regions according their functionality in the \acrshort{MD} simmulations. We denote a subsection of the sheet ends, with respect to one direction, as so-called \textit{pull blocks}, which is reserved for the application of normal load, stretching and
dragging of the sheet, and for applying the thermostat. The remaining \textit{inner
sheet} is left for the kirigami cuts and are simulated as an $NVE$ ensemble. The pull blocks are equally split between a thermostat part and a rigid part. The rigid part is only initially thermolized during relaxtion period before it is then made rigid for the final duration of the simulation. Note that the rigid parts on both sides of the sheet is then considered as a single rigid object even though
...
The full sheet has the default size $\sim 130 \times \SI{163}{\text{Å}}$ while the substrate is scaled accordingly to the sheet. This is further specified in \cref{sec:substrate}
...
The substrate is scaled in the y-dimensions to accomdate the needed space for stretching the sheet to a certain degree. In the worst case with an expected stretch amount of 200 \% the system size is roughly XXX atoms with subdivisions shown in \cref{tab:system_count}. The system is periodic in the x-y-directins such that the substrate is effectively infinite. 

with subdivision measures as shown in \cref{tab:sheet_dim}.
...
%   Substrate & $115.89 \ \%_{x_s} \: \times \:$ & $81.40 \ \%_{y_s}$ &  $150.709  \: \times \:$ & $509.152$ Å & $\phantom{2\times} 76,733.789$\\ \hline
% \end{tabular}
% \end{center}
% \end{table}
...
\begin{table}[H]
  \begin{center}
  \caption{x-y dimenstions of sheet and substrate with no cuts and assuming an expected stretch of 200\%.}
  \label{tab:sheet_dim}
  \begin{tabular}{ | l | r@{}l | r@{}l | c |} \hline
    \textbf{Region} & \multicolumn{2}{c|}{$x$-,$y$-dim} & \multicolumn{2}{c|}{Dim
    [Å]} & Area [Å$^2$]\\ \hline
  Full sheet & $x_S \: \times \: $ & $y_S$ &  $130.029 \: \times \:$ & $163.219$ Å & $\phantom{2\times} 21,223.203$ \\ \hline
...
\begin{table}[H]
  \begin{center}
  \caption{Amount of atoms in the various system regions in the case of no cutting applied to the sheet and assuming a expected stretch of 200 \%.}
  ...
  \begin{tabular}{ |c|| c | c | c | c | c | c |} \hline
    ...
    \multirow{2}{*}{Substrate} & \multirow{2}{*}{47376} & Upper & 15792 & 15792 &
    0 & 0 \\ %\hline
    & & Middle & 15792 & 0 & 15792 & 0 \\ %\hline
    & & Bottom & 15792 & 0 & 0 & 15792 \\ \hline \hline   
...
% The substrate amount should be a range !!!!!
...
corresponing to 10.86 Å. The x-y dimensions is choosen to match the dimensions
of the sheet after it is stretched. That is, we define a margin between the
...
for the margin choice is half the cut-off distance for the Tersoff potential,
...
second time. We choose a y-margin of 15 Å for which the chosen sliding speed of
$\SI{20}{m/s} = \SI{2}{Å/ps}$ gives \SI{15}{ps} of relaxtion time between encounters with the sheet similar to the inital relaxtion time. 
